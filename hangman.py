...
while run:
    ...
    # Add image file
    bg = PhotoImage(file="./Fundo/bg.png")
    ...
    for let in al:
        exec('{}=PhotoImage(file="./Letras/{}.png")'.format(let,let))
        
    # hangman images
    h123 = ['h1','h2','h3','h4','h5','h6','h7']
    for hangman in h123:
        exec('{}=PhotoImage(file="./Forca/{}.png")'.format(hangman,hangman))
    ...
    for p1 in han:
        exec('{}=Label(root,bg="#639bff",image={})'.format(p1[0],p1[1]))
    ...
    def close():
        global run
        answer = messagebox.askyesno('ALERTA','Você realmente deseja sair?')
        if answer == True:
            run = False
            root.destroy()
            
    e1 = PhotoImage(file = './Letras/exit.png')
    ex = Button(root,bd = 0,command = close,bg="#639bff",activebackground = "#639bff",font = 10,image = e1)
    ex.place(x=770,y=10)
    s2 = 'Pontuação:'+str(score)
    s1 = Label(root,text = s2,bg = "#639bff",font = ("arial",25))
    s1.place(x = 10,y = 10)
    ...
    def check(letter,button):
        ...
        if letter in selected_word:
            ...
            if win_count == len(selected_word):
                score += 1
                answer = messagebox.askyesno('Fim de jogo','Você ganhou!\nDeseja jogar novamente?')
                if answer == True:
                    run = True
                    root.destroy()   
                else:
            ...
            exec('c{}.place(x={},y={})'.format(count+1,300,-120))
            if count == 6:
                answer = messagebox.askyesno('Fim de jogo!',f'Você perdeu!\nA palavra correta era: {selected_word}\nDeseja jogar novamente?')
                if answer == True:
                    run = True
                    score = 0
                    root.destroy()
